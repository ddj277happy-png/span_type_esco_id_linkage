...
组员已经用 DeepSeek 对 5,140 条出海岗位数据做了 LKST 4 维技能标注,产出 **90,625 个 (span, L/K/S/T) 标注**。但这些 span 还是**自由文本**,没法直接对接下游系统。
...
依据 8-15 项目组会议纪要(陈童幸夫 + 张浩洋):
- 数据量仅 5,140,**训练 BERT 会被精度拖累**(实测仅 0.3)
- 决策:**放弃 BERT 训练,改用大模型直接抽 + 算法匹配 ESCO**
- 成功标准:**"若匹配效果达标,将直接放弃训练 BERT 模型"**
...
| `a_prime_summary.md` | 项目交付报告 | 数字+话术,直接给团队看 |
...
| `_02_prep_spans.py` | 组员 CSV → spans_unique.csv |
...
依据会议拍板("若匹配效果达标,将直接放弃训练 BERT 模型"),**本项目为 BERT 训练画上句号,匹配算法成为生产方案**。
